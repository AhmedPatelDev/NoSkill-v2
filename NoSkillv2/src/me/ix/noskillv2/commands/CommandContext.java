...
public class CommandContext {
	...
	public User getUser() {
		if (e instanceof SlashCommandInteractionEvent) {
			return ((SlashCommandInteractionEvent) e).getUser();
		}
		if (e instanceof MessageReceivedEvent) {
			return ((MessageReceivedEvent) e).getAuthor();
		}
		return null;
	}
	...
	public void sendFile(String message, File file) {
		...
		FileUpload upload = FileUpload.fromData(file, "image.png");
		
		if (e instanceof MessageReceivedEvent) {
			((MessageReceivedEvent) e).getMessage().reply(message).addFiles(upload).queue();
		}
		if (e instanceof SlashCommandInteractionEvent) {
			((SlashCommandInteractionEvent) e).reply(message).queue();
		}
